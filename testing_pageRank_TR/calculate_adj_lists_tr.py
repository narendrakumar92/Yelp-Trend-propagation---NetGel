...
database_name = run_details_hash['database_file_name']
User_id_Adj = run_details_hash['userid_topic_scores_file_name']
User_list = run_details_hash['userid_mapping_list_file_name']

Adjacency_file = run_details_hash["run_details"][run_number]["adjacency_file_name"]
print "current adj file location = " + Adjacency_file
output_file = run_details_hash["run_details"][run_number]["adjacency_output_prefix"]
...
class graph:
    ...
    def similarityComputation(self, con):
        ...
        nparr = np.loadtxt(Adjacency_file, dtype=int)
        adj_matrix = nparr.tolist()
        # print adj_matrix
        # pprint.pprint(adj_matrix[1048])
        npar = np.loadtxt(User_id_Adj, dtype=int)
        user_matrix = npar.tolist()
        # print user_matrix
        user_list_npar = np.loadtxt(User_list, dtype=str)
        ...
        with con:
            ...
            for topiciterator in range(0, 10):
                ...
                filename_T = output_file + str(topiciterator) + ".out"
    ...
    def Kmeans_clusterSeparate(self, filename, cluster_size):
        ...
        for i in range(0, cluster_size):
            ...
            for dataout in cluster_data:
                ...
                line = fo.write(str(index_val[count_iter]) + ":" + str(dataout[0]) + ":" + str(dataout[1]) + ":" + str(
                    dataout[2]) + "\n")
                count_iter += 1
            fo.close()
